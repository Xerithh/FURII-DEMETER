...
<VCard class="mb-6 overflow-hidden welcome-card" style="position: relative;">
  ...
  <div class="particles-container">
    <div v-for="i in 12" :key="i" class="particle" :class="`particle-${i}`" />
  </div>
  ...
  <VRow no-gutters style="position: relative; z-index: 2;">
    <VCol cols="12" sm="8" order="2" order-sm="1">
      <VCardItem class="pb-2 pt-6 px-6">
        <div class="d-flex align-center gap-3 mb-2">
          ...
          <div>
            <p class="text-xs text-disabled mb-0 text-uppercase font-weight-bold letter-spacing">
      ...
      <VCardText class="px-6 pb-6">
        <p class="mb-1 text-body-1">
          <strong class="text-primary counter-value">{{ displayedUsers }}</strong>
        ...
        <p class="text-disabled mb-5">
          ...
          Taux de réussite : {{ ((adminStore.dashboard?.tauxReussite || 0) * 100).toFixed(1) }}%
        </p>
        <div class="d-flex flex-wrap gap-3">
          <VBtn color="primary" size="small" to="/admin/questions" class="btn-shine">
            <VIcon icon="bx-help-circle" start />
            Gérer les Questions
          </VBtn>
          <VBtn variant="tonal" color="primary" size="small" to="/admin/users">
    ...
    <VCol cols="12" sm="4" order="1" order-sm="2" class="d-flex align-end justify-center">
      <img
        :src="illustration"
        style="height: 160px; width: auto; object-fit: contain; display: block;"
...
<VRow class="mb-6">
  ...
  <VCard height="100%">
    ...
    <VDivider />
    <VCardText class="d-flex flex-column align-center justify-center" style="min-height: 280px">
        ...
        <apexchart
          ...
          :options="{
            ...
            plotOptions: {
              radialBar: {
                hollow: { size: '65%' },
                track: { background: 'rgba(0,0,0,0.08)', strokeWidth: '100%' },
                dataLabels: {
                  name: {
                    show: true,
                    fontSize: '14px',
          ...
          :series="[Math.round((adminStore.dashboard?.tauxReussite || 0) * 10)]"
        />
      </div>
  ...
  <VCol cols="12" md="8">
    <VCard>
      ...
      <VCardText>
        ...
        >
          <VIcon icon="bx-bar-chart-alt-2" size="48" color="disabled" class="mb-3" />
        ...
        <apexchart
          ...
          :options="{
            ...
            dataLabels: { enabled: false },
            xaxis: { categories: adminStore.dashboard.competencesTopPerformance.slice(0,5).map(c => c.nom.substring(0, 12)) },
            grid: { borderColor: 'rgba(0,0,0,0.05)' },
            legend: { position: 'top' },
          }"
          :series="[
            { name: 'Score Moyen', data: adminStore.dashboard.competencesTopPerformance.slice(0,5).map(c => Math.round(c.scoreMoyen * 100) / 100) },
            { name: 'Taux Acquisition', data: adminStore.dashboard.competencesTopPerformance.slice(0,5).map(c => Math.round(c.tauxAcquisition * 100)) },
          ]"
        />
      </VCardText>
    </VCard>
...
const refresh = () => { adminStore.fetchDashboard() }
</script>
...
.particle-1  { left: 10%; top: 20%; animation-delay: 0s;   width: 6px;  height: 6px;  background: #7367F0; }
.particle-2  { left: 25%; top: 60%; animation-delay: 0.5s; width: 8px;  height: 8px;  background: #28C76F; }
.particle-3  { left: 40%; top: 15%; animation-delay: 1s;   width: 5px;  height: 5px;  background: #FF9F43; }
.particle-4  { left: 55%; top: 75%; animation-delay: 1.5s; width: 7px;  height: 7px;  background: #7367F0; }
.particle-5  { left: 70%; top: 30%; animation-delay: 2s;   width: 6px;  height: 6px;  background: #28C76F; }
.particle-6  { left: 85%; top: 55%; animation-delay: 2.5s; width: 8px;  height: 8px;  background: #FF9F43; }
.particle-7  { left: 15%; top: 80%; animation-delay: 3s;   width: 5px;  height: 5px;  background: #7367F0; }
.particle-8  { left: 35%; top: 45%; animation-delay: 3.5s; width: 7px;  height: 7px;  background: #28C76F; }
.particle-9  { left: 60%; top: 10%; animation-delay: 4s;   width: 6px;  height: 6px;  background: #FF9F43; }
.particle-10 { left: 75%; top: 65%; animation-delay: 4.5s; width: 8px;  height: 8px;  background: #7367F0; }
.particle-11 { left: 90%; top: 25%; animation-delay: 5s;   width: 5px;  height: 5px;  background: #28C76F; }
.particle-12 { left: 50%; top: 90%; animation-delay: 5.5s; width: 7px;  height: 7px;  background: #FF9F43; }
...
@keyframes float-particle {
  ...
  75%       { transform: translateY(10px) translateX(-10px); opacity: 0.2; }
}
